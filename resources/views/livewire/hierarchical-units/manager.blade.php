<div>
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="bg-white p-6 sm:px-8 rounded-xl shadow-sm border border-gray-200">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Arrastra el fondo para desplazarte. Las columnas organizan las
                        unidades por su nivel de profundidad funcional.</p>
                </div>
        ...
        <div x-data="hospitalBoard(@js($relationsMap))" class="bg-slate-50 border border-gray-200 rounded-xl pt-6 overflow-hidden">
            <div x-ref="scrollContainer" @mousedown="startDrag" @mouseleave="endDrag" @mouseup="endDrag"
                ...
                @foreach($groupedUnits as $level => $units)
                    <div wire:key="col-level-{{ $level }}"
                        class="snap-start shrink-0 w-72 bg-gray-100/60 rounded-xl p-3 border border-gray-200 flex flex-col gap-3">
                        ...
                        @foreach($units as $unit)
                            <div wire:key="unit-{{ $unit->id }}" @mouseenter="hoverNode({{ $unit->id }})"
                                @mouseleave="clearHover()" @click="$wire.openPanel({{ $unit->id }})"
                                ...
                                <span x-show="isParent({{ $unit->id }})" x-cloak
                                    class="absolute -top-2.5 right-2 bg-amber-100 text-amber-800 border border-amber-200 text-[9px] px-2 py-0.5 rounded-md font-bold uppercase tracking-wider shadow-sm z-10 transition-opacity">
                                    Padre
                                </span>
                                <span x-show="isChild({{ $unit->id }})" x-cloak
                                    class="absolute -top-2.5 right-2 bg-emerald-100 text-emerald-800 border border-emerald-200 text-[9px] px-2 py-0.5 rounded-md font-bold uppercase tracking-wider shadow-sm z-10 transition-opacity">
                                    Hijo
                                </span>
                ...
                <div x-show="show" x-transition:enter="transform transition ease-in-out duration-500 sm:duration-700"
                    ...
                    <div class="relative flex-1 px-6 py-6 overflow-y-auto">
                        <form wire:submit="save" id="unit-form" class="space-y-6">
                            <div>
                                <label
                                    class="block text-[11px] font-bold text-gray-500 font-secondary uppercase tracking-wider mb-1">Nombre
                                    de la Unidad</label>
                                <input wire:model="alias" type="text"
                                    class="block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-cyan focus:ring-brand-cyan sm:text-sm transition-colors">
                                @error('alias') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label
                                    class="block text-[11px] font-bold text-gray-500 font-secondary uppercase tracking-wider mb-1">Categoría
                                    / Tipo</label>
                            ...
                            @if($isServicioSelected)
                                <div class="p-4 bg-cyan-50 rounded-md border border-cyan-100">
                                    ...
                                    <select wire:model="clinical_specialty_id"
                                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-cyan focus:ring-brand-cyan sm:text-sm">
                            ...
                            <div>
                                <label
                                    class="block text-[11px] font-bold text-gray-500 font-secondary uppercase tracking-wider mb-2">Dependencia
                                    Funcional (Padres)</label>
                                <div class="mb-2">
                                    <input wire:model.live="search_parents" type="text" placeholder="Buscar unidades..."
                                        class="block w-full rounded-md border-gray-200 bg-gray-50 text-sm py-1.5 focus:bg-white focus:border-brand-cyan focus:ring-brand-cyan transition-colors">
                                </div>
                                <div
                            ...
                            <div>
                                <label
                                    class="block text-[11px] font-bold text-gray-500 font-secondary uppercase tracking-wider mb-1">Reporta
                                    Estadísticamente a</label>
                                <select wire:model="hierarchical_unit_id_to_report"
                                    class="block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-cyan focus:ring-brand-cyan sm:text-sm">
                    ...
                    <div class="border-t border-gray-200 px-6 py-4 bg-gray-50">
                        <div class="flex flex-col gap-3">
                            <button type="submit" form="unit-form"
                                class="w-full justify-center items-center px-4 py-2 border border-transparent text-sm font-bold rounded-md text-white bg-brand-cyan hover:bg-cyan-600 focus:outline-none transition-colors uppercase tracking-wide shadow-sm">
                            ...
                            @if($is_editing)
                                <button type="button" wire:click="delete"
                                    ...
                                    class="w-full justify-center items-center px-4 py-2 border border-gray-300 text-sm font-bold rounded-md text-red-600 bg-white hover:bg-red-50 focus:outline-none transition-colors uppercase tracking-wide">
                                    Eliminar Unidad
                                </button>
                            @endif
                        </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        // Recibimos el mapa inicial desde PHP (initialMap)
        Alpine.data('hospitalBoard', (initialMap) => ({
            ...
            init() {
                // Escuchamos el evento que despacha Livewire al crear/editar/borrar
                // Esto actualiza la variable en tiempo real SIN interferir con el servidor
