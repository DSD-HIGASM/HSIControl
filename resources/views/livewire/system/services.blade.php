<div class="py-8 relative">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                <x-banner text="Servicios hospitalarios"></x-banner>
                <p class="font-secondary text-brand-gray-custom mt-2">Catálogo general de servicios del hospital.</p>
            </div>
                ...
                <div class="bg-white shadow-sm rounded-xl border-t-4 border-brand-pink overflow-hidden sticky top-6">
                    <div class="p-6">
                        ...
                        <form wire:submit="save" class="space-y-4">
                            <div>
                                <label for="name" class="block font-secondary text-sm font-medium text-gray-700">Nombre del Servicio</label>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-brand-soft-100/30">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider font-secondary">Servicio</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($services as $service)
                                    <tr class="hover:bg-gray-50 transition-colors {{ $service->trashed() ? 'bg-gray-50/50 opacity-60' : '' }}" wire:key="{{ $service->id }}">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-brand-gray-custom font-secondary">
                                            #{{ $service->id }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-2">
                                                <div class="text-sm font-medium text-gray-900 {{ $service->trashed() ? 'line-through' : '' }}">{{ $service->name }}</div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end gap-3">
                                                <button wire:click="edit({{ $service->id }})" class="text-brand-blue hover:text-brand-blue-dark transition-colors" title="Editar">
                                                    <x-heroicon-o-pencil-square class="w-5 h-5" />
                                                </button>
                                                
                                                @if($service->trashed())
                                                    <button wire:click="restore({{ $service->id }})" class="text-green-600 hover:text-green-800 transition-colors" title="Reactivar">
                                                        <x-heroicon-o-arrow-path class="w-5 h-5" />
                                                    </button>
                                                @else
                                                    <button wire:click="confirmDelete({{ $service->id }})" class="text-brand-pink hover:text-red-700 transition-colors" title="Desactivar">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="px-6 py-8 text-center text-gray-500 font-secondary text-sm">
                                            No hay servicios cargados en el sistema.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm font-secondary text-gray-500">
                                        ¿Estás seguro de que deseás desactivar este servicio? Quedará inactivo y no se podrá asignar a nuevos agentes hasta que lo reactives.
                                    </p>
                                </div>
                            </div>
                        </div>
