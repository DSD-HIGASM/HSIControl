<div class="py-8 relative">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                <x-banner text="Especialidades"></x-banner>
                <p class="font-secondary text-brand-gray-custom mt-2">Catálogo de especialidades médicas y profesionales del hospital.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white shadow-sm rounded-xl border-t-4 border-brand-cyan overflow-hidden sticky top-6">
                    <div class="p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                            <x-heroicon-o-star class="w-5 h-5 text-brand-cyan" />
                            {{ $editId ? 'Editar Especialidad' : 'Nueva Especialidad' }}
                        </h3>

                        <form wire:submit="save" class="space-y-4">
                            <div>
                                <label for="name" class="block font-secondary text-sm font-medium text-gray-700">Nombre de la Especialidad</label>
                                <input wire:model="name" type="text" id="name" placeholder="Ej: CARDIOLOGÍA, PEDIATRÍA" 
                            ...
                            <div class="pt-2 flex gap-2">
                                <button type="submit" class="flex-1 flex justify-center items-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-brand-cyan hover:bg-brand-cyan-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-cyan transition-colors">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-brand-soft-100/30">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider font-secondary">Especialidad</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($specialties as $specialty)
                                    <tr class="hover:bg-gray-50 transition-colors {{ $specialty->trashed() ? 'bg-gray-50/50 opacity-60' : '' }}" wire:key="{{ $specialty->id }}">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-brand-gray-custom font-secondary">
                                            #{{ $specialty->id }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-2">
                                                <div class="text-sm font-medium text-gray-900 {{ $specialty->trashed() ? 'line-through' : '' }}">{{ $specialty->name }}</div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end gap-3">
                                                <button wire:click="edit({{ $specialty->id }})" class="text-brand-blue hover:text-brand-blue-dark transition-colors" title="Editar">
                                                    <x-heroicon-o-pencil-square class="w-5 h-5" />
                                                </button>
                                                
                                                @if($specialty->trashed())
                                                    <button wire:click="restore({{ $specialty->id }})" class="text-green-600 hover:text-green-800 transition-colors" title="Reactivar">
                                                        <x-heroicon-o-arrow-path class="w-5 h-5" />
                                                    </button>
                                                @else
                                                    <button wire:click="confirmDelete({{ $specialty->id }})" class="text-brand-pink hover:text-red-700 transition-colors" title="Desactivar">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm font-secondary text-gray-500">
                                        ¿Estás seguro de que deseás desactivar esta especialidad? Quedará inactiva y no se podrá asignar a nuevos agentes hasta que la reactives.
                                    </p>
                                </div>
                            </div>
                        </div>
